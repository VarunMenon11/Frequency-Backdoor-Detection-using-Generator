# Kaggle/Colab Notebook: CIFAR-100 Trigger Strength Ablation
...
```text
experiments/ablation_cifar100_strength/
...
!python -m scripts.train_suspicious_classifier \
  ...
  --output-dir experiments/ablation_cifar100_strength_smoke/alpha_0_04/suspicious_classifier \
  --epochs 1 \
  --batch-size 64 \
  --num-workers 2 \
  --poison-ratio 0.12 \
...
for tag, strength in strengths:
    print("=" * 80)
    print("Running CIFAR-100 strength ablation:", strength)
    print("=" * 80)

    !python -m scripts.train_suspicious_classifier \
      --zip-path {CIFAR_PATH} \
      --output-dir experiments/ablation_cifar100_strength/alpha_{tag}/suspicious_classifier \
      --epochs 10 \
      --batch-size 128 \
      --num-workers 2 \
      --poison-ratio 0.12 \
    ...
    !python -m scripts.train_spectral_generator \
      --zip-path {CIFAR_PATH} \
      --classifier-checkpoint experiments/ablation_cifar100_strength/alpha_{tag}/suspicious_classifier/suspicious_classifier.pt \
      --output-dir experiments/ablation_cifar100_strength/alpha_{tag}/spectral_generator \
      --epochs 20 \
      --batch-size 128 \
      --num-workers 2 \
      --target-label 0 \
    ...
    !python -m scripts.repair_suspicious_classifier \
      --zip-path {CIFAR_PATH} \
      --classifier-checkpoint experiments/ablation_cifar100_strength/alpha_{tag}/suspicious_classifier/suspicious_classifier.pt \
      --generator-checkpoint experiments/ablation_cifar100_strength/alpha_{tag}/spectral_generator/spectral_generator.pt \
      --output-dir experiments/ablation_cifar100_strength/alpha_{tag}/repaired_classifier \
      --epochs 3 \
      --batch-size 128 \
      --num-workers 2 \
      --target-label 0 \
    ...
    !python -m scripts.final_cifar100_evaluation \
      --zip-path {CIFAR_PATH} \
      --suspicious-checkpoint experiments/ablation_cifar100_strength/alpha_{tag}/suspicious_classifier/suspicious_classifier.pt \
      --generator-checkpoint experiments/ablation_cifar100_strength/alpha_{tag}/spectral_generator/spectral_generator.pt \
      --repaired-checkpoint experiments/ablation_cifar100_strength/alpha_{tag}/repaired_classifier/repaired_classifier.pt \
      --output-dir outputs/ablation_cifar100_strength/alpha_{tag} \
      --batch-size 256 \
      --num-workers 2 \
      --target-label 0 \
      --strength {strength} \
...
!python -m scripts.summarize_cifar100_strength_ablation \
  --ablation-output-root outputs/ablation_cifar100_strength \
  --output-json outputs/ablation_cifar100_strength/strength_ablation_summary.json \
  --output-md outputs/ablation_cifar100_strength/strength_ablation_summary.md
```
...
print(Path("outputs/ablation_cifar100_strength/strength_ablation_summary.md").read_text())
```

## 8. Zip Results For Download

```python
!zip -r cifar100_strength_ablation_results.zip \
  experiments/ablation_cifar100_strength \
  outputs/ablation_cifar100_strength
```
...
- If `alpha=0.04` has low suspicious ASR, the trigger may be too weak for the
  classifier to learn reliably.
- If `alpha=0.12` has high suspicious ASR but the generator and repair still
