...
def main() -> None:
    ...
    train_rows, train_counts = select_suspicious_training_rows(rows)
    ...
    validation_trigger_names = sorted(
        {
            str(row["variant_name"])
            for row in rows
            if row["protocol_role"] == "validation"
        }
    )
    ...
    print("Validation triggers:", ", ".join(validation_trigger_names))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        mean_seen_asr = sum(metric["target_rate"] for metric in validation_asr.values()) / len(
            validation_asr
        )
        selection_score = harmonic_mean(validation_clean["accuracy"], mean_seen_asr)
        epoch_record = {
            "epoch": epoch,
...
def evaluate_final_test(
    *,
    ...
    for name in test_trigger_names:
        ...
        seen = bool(trigger_rows[0]["seen_during_defense_training"])
    ...
    return {
        ...
        "mean_seen_trigger_asr": sum(seen_values) / len(seen_values),
        "mean_held_out_trigger_asr": sum(held_out_values) / len(held_out_values),
        "trigger_results": trigger_results,
        "interpretation_note": (
            "This is the intentionally backdoored pretrained classifier baseline, "
            "not the defended model. High ASR for seen attack triggers verifies that "
            "poisoning succeeded; held-out trigger ASR measures attack transfer."
        ),
    }
