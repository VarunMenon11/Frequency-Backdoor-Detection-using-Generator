"""Run a separate 64x64 Tiny ImageNet frequency-backdoor experiment.

This Kaggle-oriented script keeps all outputs separate from the CIFAR-100 and
STL-10 experiments. It trains a suspicious classifier, a spectral correction
generator, a repaired classifier, and a final report for Tiny ImageNet.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--data-root",
        type=Path,
        default=Path("/kaggle/input/tiny-imagenet"),
        help="Path containing tiny-imagenet-200, or the tiny-imagenet-200 folder itself.",
    )
    parser.add_argument("--experiment-root", type=Path, default=Path("experiments/tinyimagenet_64x64"))
    parser.add_argument("--output-root", type=Path, default=Path("outputs/tinyimagenet_64x64"))
    ...
    parser.add_argument("--horizontal-frequency", type=int, default=12)
    parser.add_argument("--vertical-frequency", type=int, default=12)
...
def main() -> None:
    ...
    train_clean, val_clean, label_names = load_tiny_imagenet(args.data_root)
    ...
    triggered_val = TriggeredVisionDataset(
        val_clean,
        target_label=args.target_label,
        trigger_config=trigger_config,
    )
    ...
    print("Validation samples:", len(val_clean))
    print("Triggered ASR samples:", len(triggered_val))
    ...
    final_json = args.output_root / "final_tinyimagenet_64x64_summary.json"
    final_md = args.output_root / "final_tinyimagenet_64x64_report.md"
...
def load_tiny_imagenet(data_root: Path) -> tuple[Dataset, Dataset, list[str]]:
    root = find_tiny_imagenet_root(data_root.expanduser())
    train_dir = root / "train"
    val_dir = root / "val"
    words_path = root / "words.txt"
    val_annotations = val_dir / "val_annotations.txt"
    if not train_dir.exists() or not val_annotations.exists():
        raise RuntimeError(
            "Could not find Tiny ImageNet train/val structure. Set --data-root to "
            "the folder containing tiny-imagenet-200, or to tiny-imagenet-200 itself."
        )

    transform = transforms.ToTensor()
    train_dataset = datasets.ImageFolder(str(train_dir), transform=transform)
    class_to_idx = train_dataset.class_to_idx
    label_names = build_label_names(train_dataset.classes, words_path)
...
def find_tiny_imagenet_root(path: Path) -> Path:
    if (path / "train").exists() and (path / "val").exists():
        return path
    if (path / "tiny-imagenet-200").exists():
        return path / "tiny-imagenet-200"
    for candidate in path.rglob("tiny-imagenet-200") if path.exists() else []:
        if (candidate / "train").exists() and (candidate / "val").exists():
            return candidate
    for candidate in path.rglob("train") if path.exists() else []:
        possible_root = candidate.parent
        if (possible_root / "val" / "val_annotations.txt").exists():
            return possible_root
    return path


def build_label_names(wnids: list[str], words_path: Path) -> list[str]:
    words: dict[str, str] = {}
    if words_path.exists():
        for line in words_path.read_text(encoding="utf-8").splitlines():
            parts = line.split("\t", maxsplit=1)
            if len(parts) == 2:
                words[parts[0]] = parts[1].split(",", maxsplit=1)[0]
    return [f"{wnid}:{words.get(wnid, wnid)}" for wnid in wnids]
...
class TinyImageNetValDataset(Dataset):
    def __init__(
        ...
        *,
        ...
        self.image_dir = val_dir / "images"
        self.transform = transform
        self.samples: list[tuple[Path, int]] = []
        for line in val_annotations_path.read_text(encoding="utf-8").splitlines():
            parts = line.split("\t")
            if len(parts) < 2:
                continue
            ...
            if wnid not in class_to_idx:
                continue
            self.samples.append((self.image_dir / image_name, class_to_idx[wnid]))
        if not self.samples:
            raise RuntimeError("Tiny ImageNet validation annotations produced no samples")
...
def train_suspicious_classifier(
    *,
    ...
    clean_loader = make_loader(clean_val, args.batch_size, args.num_workers, shuffle=False, device=device)
    asr_loader = make_loader(triggered_val, args.batch_size, args.num_workers, shuffle=False, device=device)
...
def train_generator(
    *,
    ...
    val_loader = make_loader(val_clean, args.batch_size, args.num_workers, shuffle=False, device=device)
    classifier.eval()
    for parameter in classifier.parameters():
        parameter.requires_grad = False
    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.AdamW(generator.parameters(), lr=args.generator_learning_rate, weight_decay=1e-5)
    history = []
    start = perf_counter()
...
def repair_classifier(
    *,
    ...
    asr_loader = make_loader(triggered_val, args.batch_size, args.num_workers, shuffle=False, device=device)
    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.AdamW(repaired.parameters(), lr=args.repair_learning_rate, weight_decay=args.weight_decay)
    history = []
    start = perf_counter()
...
def repair_epoch(
    *,
    ...
    for batch_index, (clean_images, clean_labels) in enumerate(loader):
        ...
        clean_labels = clean_labels.to(device)
        non_target_mask = clean_labels != target_label
        repair_images_list = [clean_images]
        repair_labels_list = [clean_labels]
        if int(non_target_mask.sum().item()) > 0:
            ...
            repair_images_list.extend([corrected_images, triggered_images])
            repair_labels_list.extend([non_target_labels, non_target_labels])
        repair_images = torch.cat(repair_images_list, dim=0)
        repair_labels = torch.cat(repair_labels_list, dim=0)
        optimizer.zero_grad(set_to_none=True)
...
def final_evaluation(
    *,
    ...
    return {
        "dataset": {"name": "Tiny ImageNet", "image_shape": [3, 64, 64], "classes": label_names},
        "trigger": {
...
def save_final_panels(
    *,
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    records = []
    for index in range(len(clean_val)):
        clean_image, clean_label = clean_val[index]
        ...
        panel_path = save_correction_panel(
            ...
            output_dir / f"tinyimagenet_64_panel_val_index_{index}.png",
            columns=4,
        )
...
def render_markdown(summary: dict) -> str:
    ...
    return f"""# Final Tiny ImageNet 64x64 Frequency-Backdoor Experiment
